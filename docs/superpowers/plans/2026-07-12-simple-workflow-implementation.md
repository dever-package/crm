...
- [ ] 实现按部门查询启用人员，并按“当前活跃资产最少、最近分配最早、ID”选择阶段负责人。
- [ ] 实现任务自动分配，按“待处理任务最少、最近分配最早、ID”选择负责人；同批任务逐条创建，后续选择可看到前一条负载。
- [ ] 阶段跟随任务直接分配给当前阶段负责人；手动任务保留目标部门但负责人为空。
- [ ] 手动未分配任务不再允许部门成员领取或执行，只有负责人明确后才能办理。
- [ ] 首次启动必须使用默认入口流程；自动首阶段直接分配，手动首阶段要求调用方提供负责人。
- [ ] 对修改的 Go 文件执行 `gofmt` 和 diff 自查。
- [ ] 提交：`feat: add workflow assignment engine`
...
- [ ] 删除“任务完成后自动跳阶段”；自动规则仍可自动执行，但只改变任务状态。
- [ ] 实现“完成阶段”：校验当前操作者、所有必做任务、目标阶段分配方式，然后取消未完成可选任务并进入下一阶段。
- [ ] 当前流程最后阶段完成后，仅按配置的 `next_workflow_id` 进入后续流程；无后续流程则完成整个进度。
- [ ] 实现“终止/流失”：仅当前负责人可操作，原因必填，取消待办且不进入后续流程。
- [ ] 每次进入、完成、终止、分配和改派都写入现有操作日志，包含操作者与关键前后值。
- [ ] 对修改的 Go 文件执行 `gofmt` 和 diff 自查。
- [ ] 提交：`feat: add explicit workflow transitions`
...
- [ ] 登录会话和 `/crm/work/me` 返回 `can_dispatch`。
- [ ] 客户详情返回当前流程、阶段、负责人、阶段任务、是否可完成/终止/分配等能力字段。
- [ ] 增加候选人员、分配任务、改派任务、更换阶段负责人、完成阶段和终止流程接口；API 层保持薄封装。
- [ ] 当前阶段负责人可分配本阶段的手动任务；调度员可对全部活跃业务做分配、改派、更换负责人和完成阶段。
- [ ] 调度员不能代办任务、跳过必做项、修改任务结果或代替负责人终止流程。
- [ ] 客户列表增加 `mine/all` 查询范围；非调度员强制为 `mine`。
- [ ] 对修改的 Go 文件执行 `gofmt` 和 diff 自查。
- [ ] 提交：`feat: expose workflow actions to workbench`
...
- [ ] 扩展现有类型，承接当前进度、阶段任务、候选人员和权限字段。
- [ ] 在客户资产详情的“流程”页签顶部增加单一流程操作区：进度、负责人、任务、分配/改派、完成阶段、终止。
- [ ] 当前负责人完成阶段时，自动阶段直接确认；手动阶段在同一操作中选择下一负责人。
- [ ] 调度员在现有工作台看到“我的/全部”切换和管理动作，普通员工界面不增加多余控件。
- [ ] 复用现有 `workApi`、按钮、弹层和提示组件，不把业务逻辑继续堆进 `work-auth.tsx`。
- [ ] 仅做 TypeScript/JSX 静态阅读和 diff 自查，不运行 build/test。
- [ ] 提交：`feat: add simple workflow controls to workbench`
