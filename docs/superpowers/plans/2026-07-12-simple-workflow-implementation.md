...
- [x] 为无流程的新租户提供一套可编辑默认配置：签署流程包含接单建档、资料收集、诊断核验、产品确认、合同签署、签约确认；运营流程由签署流程显式衔接。
- [x] 运行时代码不按 MKT/NPL/PM 名称硬编码部门；迁移仅按现有部门标识解析实际 ID，十一维表单和自动规则继续作为任务配置。
- [x] 检查全部 JSON 可解析、Go 文件已格式化、前端引用和导出一致、无旧自动流转调用残留。
- [x] 运行 Dever 静态审计（不运行 build/test），由现有 `dever run` 热更新重启业务进程并确认 8082 监听。
- [x] 汇总人工验证路径和已知边界。
- [x] 提交：`chore: finalize simple workflow defaults`

### 人工验证路径

1. 后台进入“流程配置”，确认可以切换流程，并在左侧维护该流程的阶段、在右侧维护当前阶段任务。
2. 将阶段分配方式分别设为自动和手动；将任务负责方式分别设为跟随阶段、自动分配和手动分配。
3. 工作台新建客户资产，确认进入唯一默认入口流程；自动阶段显示具体负责人，手动阶段要求选择负责人。
4. 当前负责人完成普通/表单/自动核验任务；必做任务未完成时不能完成阶段，全部完成后仍需点击“完成阶段”。
5. 进入手动阶段时由当前负责人选择下一负责人；协作任务按配置自动分配或由当前负责人手动分配。
6. 使用流程调度账号切换“我的/全部”，验证任务改派、阶段负责人更换和代完成阶段；确认调度员不能代办任务或代替负责人终止流程。
7. 完成签署流程最后阶段，确认进入运营流程；由当前负责人填写原因终止流程，确认不再进入后续流程。

### 已知边界

- 当前实现只支持流程内顺序阶段和一个可选后续流程，不支持条件分支、并行阶段和流程版本。
- 自动分配按当前活跃资产/待办数量、最近分配时间和人员 ID 选择，不引入排班、权重或容量上限。
- SQL 迁移负责补齐当前默认流程；业务运行时不按部门名称或 MKT/NPL/PM 角色硬编码。
